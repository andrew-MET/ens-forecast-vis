...
export class stackedProb {
    constructor(parent) {
      ...
      this._paramUnit = "";
      this._stackColours = {
        q100q80: d3.interpolateBlues(0.9),
        q80q60: d3.interpolateBlues(0.7),
        q60q40: d3.interpolateBlues(0.5),
        q40q20: d3.interpolateBlues(0.3),
        q20q0: d3.interpolateBlues(0.1)
      };
      this._bg = "#FFF";
      this._minY = null;
      this._maxY = null;
    ...
    _toQuantiles() {
      return d3.flatRollup(
        this._data,
        (v) => ({
          q100: d3.min(v.map((d) => d[this._paramKey])),
          q80: d3.quantile(
          ...
          q0: d3.max(v.map((d) => d[this._paramKey])),
    ...
    _barWidths(data, key) {
      const numBars = [...new Set(data.map((d) => d[key]))].length;
      return Math.round(
        ((this._width - this._margins.left - this._margins.right) / numBars) * 0.8
      );
    }
    ...
    _drawStack() {
      ...
      const rectData = this._toStacks();
      const barWidth = this._barWidths(rectData, "time");
      
      const rects = selectOrAppend("g", "rects", bounds)
        .selectAll("rect")
        .data(rectData)
        .join("rect")
        .attr("x", d => x(this._timeParser(d.time)) - barWidth / 2)
        .attr("width", barWidth)
        .attr("y", d => y(d.max))
        .attr("height", d => y(d.min) - y(d.max))
        .attr("fill", (d) => this._stackColours[d.range]);
      ...
      const xAxis = selectOrAppend("g", "xAxis", bounds)
        ...
        .attr(
          "transform",
          `translate(0, ${
            this._height - this._margins.bottom - this._margins.top
          })`
        )
      ...
      const tooltipReveal = (event, d) => {
        ...
        const xPos = x(d[0]);
        const dataNow = this._data.filter(
          (dd) => dd[this._validTimeKey] === d[0]
        );
        const totalMembers = dataNow.length;
        const exceedenceMembers = dataNow.filter(
          (dd) => dd[this._paramKey] >= this._threshold
        ).length;
        tooltip
          .html(
            `<div id="stack-tooltip">
                    <p style="margin: 0; font-size: 0.75em"><u>${d3.utcFormat(
                      "%H:%M %a %d %b %Y"
                    )(this._timeParser(d[0]))}</u></p>
                    <p style="margin: 0; font-size: 0.75em">${exceedenceMembers} of ${totalMembers} members &#8805 ${
              this._threshold
            }</p>
                  </div>`
          )
        ...
        new dotPlot("stack-tooltip")
          .data(dataNow)
          .paramKey(this._paramKey)
          .threshold(this._threshold)
          .step(this._binStep)
          .force(this._forceStep)
          .colours(this._stackColours.q100q80, this._stackColours.q40q20)
          .render()
      };
      ...
      const tooltipMove = (event, d) => {
        const xPos =
          x(this._timeParser(d[0])) > this._width - this._margins.left - 158
            ? -178
            : 10;
        tooltip
      ...
      if (this._threshold) {
        ...
        const threshData = [
          { x: 0, y: yVal },
          {
            x: this._width - this._margins.left - this._margins.right / 2,
            y: yVal
          }
        ];
